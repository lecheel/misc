...
sub print_matches_in_resource {
    ...
    if ( $n_before_ctx_lines || $n_after_ctx_lines ) {
        ...
        local $_;
        my $FILE = "File: ".Cwd::getcwd()."/";
        my $display_dirs=$FILE; {
	    $display_dirs = Term::ANSIColor::colored($display_dirs, $ENV{ACK_COLOR_FILENAME});
	}
        while ( <$fh> ) {
            if ( does_match($opt, $_) ) {
                if ( !$has_printed_for_this_resource ) {
			...
			if (!$fte) {
			    App::Ack::print_blank_line();
                    ...
                    if ( $print_filename && $heading ) {
			if ($fte) {
			    App::Ack::print( $display_dirs);
			    App::Ack::print_filename( $display_filename, "\n");
		        } 
			else {
			    App::Ack::print_filename( $display_filename, $ors );
			}
